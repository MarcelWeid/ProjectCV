...
| TASK-007 | ~~Local dev environment: .NET 9 SDK, Docker Desktop, Node.js LTS, PostgreSQL client~~ **Done 2026-05-13.** .NET 10 SDK (10.0.104, LTS — DEC-005), Docker Engine (29.2.1 in WSL), psql client (16.13), `dotnet ef` global tool (10.0.8) installiert. Node.js LTS bewusst auf Phase 3 verschoben. End-to-End-Test mit Npgsql gegen lokalen Postgres+pgvector erfolgreich. | — | Done | Closed |
...
| R9 | gpt-4o model retirement scheduled for 1 October 2026 (per Azure deployment metadata). MVP completes ~mid-August 2026 (within retirement window), but the bot will need migration to a successor model (newer gpt-4o version, gpt-4.1, or gpt-5-family variant) before October. Migration may require prompt tuning and re-running the eval harness. | **Monitored.** R9 logged 2026-05-13 in DEC-003. Concrete migration session scheduled for summer 2026, before retirement. Auto-upgrade is active for the deployment, which may handle minor version bumps automatically — but a full model-family switch (e.g. to gpt-4.1) remains a manual decision. NFR-031 (model swappable via config) makes the technical change cheap; the test-set re-baselining is the real cost. |
| R10 | Self-hosted database (Postgres + pgvector via Docker) shifts backup, update, and disk-integrity responsibilities from Hetzner to us. Without explicit handling, a VM outage or accidental volume loss could destroy the candidate corpus and all operational state. | **Monitored.** Backup strategy is a Phase 5 deliverable: `pg_dumpall` daily as a cron job on the VM, dump copied off the VM (e.g. to a backup target via rsync/scp, or downloaded periodically by the candidate). Snapshot of the VM (taken 2026-05-12 as `hardened-base`) covers OS-level recovery; per-day DB backups cover application-level recovery. Recovery procedure to be documented in the runbook (TASK-OR-012). |
